...
class Drawer_with_U_handle(ConceptTemplate):
    def __init__(self, number_of_drawer, drawers_params, position=[0, 0, 0], rotation=[0, 0, 0]):
        ...
        # Record Parameters
        drawer_count = int(number_of_drawer[0])
        self.number_of_drawer = [drawer_count]
        params_per_drawer = 20
        self.drawer_size = [drawers_params[i * params_per_drawer: i * params_per_drawer + 3] for i in range(drawer_count)]
        self.bottom_size = [drawers_params[i * params_per_drawer + 3] for i in range(drawer_count)]
        self.front_size = [drawers_params[i * params_per_drawer + 4: i * params_per_drawer + 7] for i in range(drawer_count)]
        self.front_offset = [drawers_params[i * params_per_drawer + 7] for i in range(drawer_count)]
        self.left_right_inner_size = [drawers_params[i * params_per_drawer + 8] for i in range(drawer_count)]
        self.rear_front_inner_size = [drawers_params[i * params_per_drawer + 9] for i in range(drawer_count)]
        self.number_of_handle = [int(drawers_params[i * params_per_drawer + 10]) for i in range(drawer_count)]
        self.handle_sizes = [drawers_params[i * params_per_drawer + 11: i * params_per_drawer + 14] for i in range(drawer_count)]
        self.handle_offset = [drawers_params[i * params_per_drawer + 14: i * params_per_drawer + 16] for i in range(drawer_count)]
        self.handle_separation = [drawers_params[i * params_per_drawer + 16] for i in range(drawer_count)]
        self.drawer_offset = [drawers_params[i * params_per_drawer + 17: i * params_per_drawer + 20] for i in range(drawer_count)]
        ...
        for drawer_idx in range(self.number_of_drawer[0]):
            for mesh_idx in range(6 + self.number_of_handle[drawer_idx]):
                if mesh_idx < 2:
                    position_sign = -1 if mesh_idx == 0 else 1
                    mesh_position = [position_sign * (
                    ...
                    handle_center = [self.drawer_offset[drawer_idx][0] + self.handle_offset[drawer_idx][0] +
                                     position_sign * self.handle_separation[drawer_idx] / 2,
                                     self.drawer_offset[drawer_idx][1] + self.handle_offset[drawer_idx][1],
                                     self.drawer_offset[drawer_idx][2] + self.drawer_size[drawer_idx][2] / 2 +
                                     self.front_size[drawer_idx][2] + self.front_size[drawer_idx][2] / 2]

                    handle_width = self.handle_sizes[drawer_idx][0]
                    handle_height = self.handle_sizes[drawer_idx][1]
                    handle_thickness = self.handle_sizes[drawer_idx][2]
                    u_handle_rotation = [np.pi / 2, 0, 0]
                    rot_mat = get_rodrigues_matrix([1, 0, 0], np.pi / 2)

                    # Use the same cross section for all three bars.
                    u_vertices_list = []
                    u_faces_list = []
                    u_total_num_vertices = 0

                    for bar_sign in [-1, 1]:
                        local_offset = np.array([
                            bar_sign * (handle_width / 2 - handle_thickness / 2),
                            0,
                            0,
                        ])
                        world_offset = np.matmul(rot_mat, local_offset)
                        u_mesh_position = [
                            handle_center[0] + world_offset[0],
                            handle_center[1] + world_offset[1],
                            handle_center[2] + world_offset[2],
                        ]
                        u_mesh = Cuboid(
                            handle_height,
                            handle_thickness,
                            handle_thickness,
                            position=u_mesh_position,
                            rotation=u_handle_rotation,
                        )
                        u_vertices_list.append(u_mesh.vertices)
                        u_faces_list.append(u_mesh.faces + u_total_num_vertices)
                        u_total_num_vertices += len(u_mesh.vertices)

                    local_offset = np.array([0, handle_height / 2 - handle_thickness / 2, 0])
                    world_offset = np.matmul(rot_mat, local_offset)
                    u_mesh_position = [
                        handle_center[0] + world_offset[0],
                        handle_center[1] + world_offset[1],
                    ...
                    self.mesh = type('tmp', (), {})()
                    self.mesh.vertices = np.concatenate(u_vertices_list)
                    self.mesh.faces = np.concatenate(u_faces_list)
