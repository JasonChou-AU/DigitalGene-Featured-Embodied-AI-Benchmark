...
def _bind_preview_texture(stage, mesh_prim, texture_path):
    looks_scope = UsdGeom.Scope.Define(stage, "/Bottle/Looks")
    mat = UsdShade.Material.Define(stage, f"{looks_scope.GetPath()}/BottleMat")
    shader = UsdShade.Shader.Define(stage, f"{looks_scope.GetPath()}/BottleMat/PBRShader")
    shader.CreateIdAttr("UsdPreviewSurface")
    shader.CreateInput("roughness", Sdf.ValueTypeNames.Float).Set(0.5)
    shader.CreateInput("metallic", Sdf.ValueTypeNames.Float).Set(0.0)

    st_reader = UsdShade.Shader.Define(stage, f"{looks_scope.GetPath()}/BottleMat/TexCoordReader")
    st_reader.CreateIdAttr("UsdPrimvarReader_float2")
    st_reader.CreateInput("varname", Sdf.ValueTypeNames.Token).Set("st")

    tex = UsdShade.Shader.Define(stage, f"{looks_scope.GetPath()}/BottleMat/DiffuseTex")
...
def _extract_lid_obj(concept_data):
    conceptualization = concept_data["conceptualization"]
    for c in conceptualization:
        template_name = c.get("template")
        if template_name is None:
            continue
        module = eval(template_name)
...
def _export_grasps(stage, bottle_root_path, lid_obj, scale_to_meters):
    ...
    for idx, (p1, p2) in enumerate(test_params):
        ...
        prim.CreateAttribute("grasp:approach", Sdf.ValueTypeNames.Vector3f).Set(
            _to_vec3f(spec["world_approach_direction"])
        )
...
def export_real_bottle_usda(
    ...
    _add_visual_mesh(
        stage,
        f"{bottle_root_path}/body/visual",
        body_verts,
        body_faces,
        scale_to_meters,
        uvs=body_uvs,
        face_uvs=body_face_uvs,
    )
    _bind_preview_texture(stage, stage.GetPrimAtPath(f"{bottle_root_path}/body/visual"), visual_tex_path)
    _add_collision_mesh(stage, f"{bottle_root_path}/body/collision", body_verts, body_faces, scale_to_meters)

    # Lid visual + collision
    lid_verts, lid_faces, lid_uvs, lid_face_uvs = _load_obj_vertices_faces_uv(lid_obj_path)
    _add_visual_mesh(
        stage,
        f"{bottle_root_path}/lid/visual",
        lid_verts,
        lid_faces,
        scale_to_meters,
        uvs=lid_uvs,
        face_uvs=lid_face_uvs,
    )
    _bind_preview_texture(stage, stage.GetPrimAtPath(f"{bottle_root_path}/lid/visual"), visual_tex_path)
    _add_collision_mesh(stage, f"{bottle_root_path}/lid/collision", lid_verts, lid_faces, scale_to_meters)

    # Knowledge grasps from conceptualization pkl
    concept_data = _load_concept_from_pkl(concept_pkl_path)
    lid_obj = _extract_lid_obj(concept_data)
...
def main():
    ...
    input_dir = os.path.join(repo_root, "batch_01_0002_Bottle_002_lemon_tea")
    visual_tex = os.path.join(input_dir, "texture.jpg")
    body_obj = os.path.join(input_dir, "segmentation", "body.obj")
    lid_obj = os.path.join(input_dir, "segmentation", "lid.obj")
    concept_pkl = os.path.join(script_dir, "real_bottle", "lemon_tea.pkl")
    output_usda = os.path.join(script_dir, "real_bottle", "lemon_tea_sim_ready.usda")
    scale_to_meters = 0.01
    mass_kg = 0.5
    init_pos = [0.0, 0.2, 0.0]
    init_euler = [90.0, 0.0, 0.0]

    out = export_real_bottle_usda(
